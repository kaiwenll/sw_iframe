...
char SendFileName[1024]; //存放发送的文件名称
...
void *SocketPthread_func(void *arg)
{
	/*1. 定义相关需要使用的变量*/
	unsigned int  rx_cnt;
	unsigned int  SendOkByte=0; 	   //已经发送成功的字节数
	...
	if(stat(SendFileName,&FileInfoBuf))
	{
	...
	/*3. 进入到文件发送状态*/
	memset(&RxTxData,0,sizeof(struct SocketPackageData));//初始化结构体
	RxTxData.FileSize=FileInfoBuf.st_size; //文件总大小
	strcpy(RxTxData.FileName,SendFileName); //文件名称 "\123\ 456.c"
	while(1)
	{
		/*4. 文件数据封装发送*/
		//读取文件数据
		rx_cnt=fread(RxTxData.SrcDataBuffer,1,sizeof(RxTxData.SrcDataBuffer),fp);
		RxTxData.NumCnt++; //包数量
		RxTxData.CurrentSize=rx_cnt; //当前读取的字节数
		SetDataPackage(&RxTxData);   //结构体数据封包
		SendOkByte+=rx_cnt; 		 //记录已经发送的字节数量
...
SEND_SRC_DATA: //触发重发数据的标签
		write(tcp_client_fd,&RxTxData,sizeof(struct SocketPackageData)); //发送数据
...
WAIT_ACK: //触发继续等待客户端应答
		...
		if(select_state>0)//表示有事件产生
		{
			...
			{
				//读取数据
				rx_cnt=read(tcp_client_fd,&AckData,sizeof(struct SocketAckPackageData));
				if(rx_cnt==sizeof(struct SocketAckPackageData))//等于4的情况
				{
					if(AckData.AckStat!=0x80)
					{
					...
					{
						//判断文件是否读取完毕
						if(rx_cnt!=sizeof(struct SocketAckPackageData))
						{
							printf("服务器提示:文件发送成功!\n");
							break;
						}
				...
				if(rx_cnt==0)
				{
					if(SendOkByte==RxTxData.FileSize)
					{
						printf("服务器提示:文件发送成功!\n");
					}
...
int main(int argc,char **argv)
{
	...
	strcpy(SendFileName,argv[2]);  //存放传入的文件名称	
